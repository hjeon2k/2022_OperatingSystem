...
typedef struct eos_alarm {
	...
	void *arg;
	_os_node_t alarm_queue_node; // prev, next : let's sort by _os_add_node_priority(), *ptr_data = tcb(task), priority by alarm 
} eos_alarm_t;
...
typedef struct tcb {
	int32u_t state;
	//int32u_t priority; //node's priority = tcb's priority... priority doesn't change
	addr_t sp;
	int32u_t period;
